...
function initializeActiveLinks() {
  ...
  const observer = new IntersectionObserver(
    ...
    // FIX: Changed threshold from 0 to 0.2 so it only triggers when section is genuinely visible
...
function initializeProjectSlider() {
  ...
  if (slider && slideLeft && slideRight) {
    ...
    const moveRight = () => {
      // FIX: Used Math.abs for accurate fractional pixel calculation
      let isAtEnd = Math.abs(slider.scrollWidth - slider.scrollLeft - slider.clientWidth) <= 2;
    ...
    slideLeft.addEventListener('click', () => {
      let isAtStart = slider.scrollLeft <= 2;
...
// FIX: Loader will now properly hide only when everything is loaded
